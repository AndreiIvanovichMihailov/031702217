...
#define ll long long
using namespace std;
string transfor(const wstring& s)
{
	string fallback;
	wstring_convert<codecvt_utf8<wchar_t> > wcv;
	fallback = wcv.to_bytes(s);
	return fallback;
}
wstring transfor(const string& s)
{
	wstring fallback;
	wstring_convert<codecvt_utf8<wchar_t> > wcv;
	fallback = wcv.from_bytes(s);
	return fallback;
}
struct node
{
...
int main(int argv, char** argc)
{
	...
	for (int i = 1; i <= cnt; i++)
	{
		...
		if (text.difficult == 1)
		{
			...
			temp = transfor(L"地址");
			output << "\"" << temp << "\"" << ":";
			temp = transfor(text.address1);
			output << "[\"" << temp << "\",";
			temp = transfor(text.address2);
			output << "\"" << temp << "\",";
			temp = transfor(text.address3);
			output << "\"" << temp << "\",";
			temp = transfor(text.address4);
			output << "\"" << temp << "\",";
			string total = "";
			temp = transfor(text.address5); total += temp;
			temp = transfor(text.address6); total += temp;
			temp = transfor(text.address7); total += temp;
			output << "\"" << total << "\"]" << ",";
			temp = transfor(L"姓名");
			output << "\"" << temp << "\"" << ":";
			temp = transfor(text.name);
			output << "\"" << temp << "\"" << ",";
			temp = transfor(L"手机");
			output << "\"" << temp << "\"" << ":";
			temp = transfor(text.phone);
			output << "\"" << temp << "\"}";
		}
		else
		{
			...
			temp = transfor(text.address4);
			output << "\"" << temp << "\",";
			temp = transfor(text.address5);
			output << "\"" << temp << "\",";
			temp = transfor(text.address6);
			output << "\"" << temp << "\",";
			temp = transfor(text.address7);
			output << "\"" << temp << "\"]" << ",";
			temp = transfor(L"姓名");
			output << "\"" << temp << "\"" << ":";
			temp = transfor(text.name);
